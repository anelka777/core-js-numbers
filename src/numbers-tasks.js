...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vectorsProduct = x1 * x2 + y1 * y2;
  const lenghtOne = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lenghtTwo = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = vectorsProduct / (lenghtOne * lenghtTwo);
...
function roundToPowerOfTen(num, pow) {
  ...
  const roundedNumber = Math.round(num / devider) * devider;
...
function getSumOfDigits(num) {
  const numToSt = Math.abs(num).toString();
  let sum = 0;
  for (let i = 0; i < numToSt.length; i += 1) {
    sum += parseInt(numToSt[i], 10);
  }
  return sum;
}
...
function numberToStringInBase(number, base) {
  const stringRep = number.toString(base);
  return stringRep;
}
...
function toExponential(number, fractionDigits) {
  const stringRep = number.toExponential(fractionDigits);
  return stringRep;
}
...
function toFixed(number, fractionDigits) {
  const stringRep = number.toFixed(fractionDigits);
  return stringRep;
}
...
function toPrecision(number, precision) {
  const stringRep = number.toPrecision(precision);
  return stringRep;
}
...
function getHypotenuse(a, b) {
  const lenghtOfG = Math.hypot(a, b);
  return lenghtOfG;
}
